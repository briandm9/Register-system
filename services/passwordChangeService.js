...
const handleSendPasswordEmail = async (user) => {
  ...
  if (!canChangePassword(user.lastPasswordChange)){
    return { status: 429, sucess: false, message: 'Password update limit reached. Changes are allowed only once every 7 days' };
  }
  ...
  if (!mailResult.success) {
    return { status: 500, sucess: false, message: 'Failed to send reset password email' };
  }

  user.lastEmailSent = new Date();
  await user.save();

  return { status: 200, sucess: true, message: 'Password reset email has been sent' };
};
