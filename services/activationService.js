...
const handleResendActivationEmail = async (user) => {
  ...
  if (!mailResult.success) {
    return { status: 500, success: false, message: 'New token generated, but failed to send activation email' };
  }

  user.lastEmailSent = new Date();
  await user.save();

  return { status: 200, success: true, message: 'A new activation email has been sent' };
};
